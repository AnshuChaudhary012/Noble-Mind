...
contentList.innerHTML = CONTENT_DATA.map((item, index) => {
  return `
    <div data-aos="fade-up"
      ...
      class="flex gap-[10px] items-start border border-border-vision rounded-[16px] px-3 py-3">
...
industryList.innerHTML = INDUSTRY_DATA.map((item, index) => {
  return `
    ...
    >
      ...
      <p class="mt-2 text-base leading-150 font-normal text-copy-muted">
        ${item.description}
      </p>
...
valuesList.innerHTML = VALUES_DATA.map((item, index) => {
  return `
        ...
        >
            ...
            <p class="mt-[16px] font-normal text-base leading-150 text-copy-neutral">
                  ${item.description}
                </p>
